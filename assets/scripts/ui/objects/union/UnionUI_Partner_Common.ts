...
@ccclass
export class UnionUI_Partner_Common extends BaseUI {

    protected static className = "UnionUI_Partner_Common";
    ...
    onShow()
    {
        ...
        this.node.getChildByName("label_name").getComponent(cc.Label).string = "玩家:"+GameDataManager.getInstance().userInfoData.userName
    }
    ...
    initView(playerId, playerName, type, timeIdx)
    {
        this.initDate()
        this.resetView()
        this.node.getChildByName("label_name").getComponent(cc.Label).string = Utils.getShortName(playerName,5) + "的组长统计"
    ...
    private updateTjList(dataList) // 刷新统计滚动框
    {
        ...
        for (let i = 0; i < dataList.length; ++i) {
            ...
            pnode.initView(dataList[i], "partner", this.timeIdx, this.reqPlayerId)
    ...
    private queryData()
    {
        ...
        if (this.curSelectType == "top_left") // 选择是战绩统计-组长
        {
        ...
        else if (this.curSelectType == "top_right") // 选择是联盟统计-组长
        {
    ...
    button_close() {
        ...
        UIManager.getInstance().closeUI(UnionUI_Partner_Common);
    }
