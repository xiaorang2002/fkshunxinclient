...
cc.Class({
    ...
    onLoad(){
        ...
        // this.node.addComponent('MagicQRCode')
    ...
    setContent() {
        ...
        this.graphics.rect(0-width*0.5, 0-width*0.5, width, width);
        this.graphics.fill();
        this.graphics.close();
        //生成二维码数据
        ...
        for (let row = 0; row < num; row++) {
            for (let col = 0; col < num; col++) {
                if (qrcode.isDark(row, col)) {
                    this.graphics.rect(this.margin + col * tileW-width*0.5, size - tileH - Math.round(row * tileH) + this.margin-width*0.5, w, h);
                    this.graphics.fill();
                }
            }
        }
